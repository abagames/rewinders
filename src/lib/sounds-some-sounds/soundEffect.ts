...
function createBuffers(type: Type, params, volume: number): SoundEffect {
  ...
  gainNode.gain.value = volume * masterVolume * delayMasterVolume;
...
let delayFeedbackVolume: number;
let delayMasterVolume = 1;

export function setDelay(
  _delayTime = 0.01,
  _delayFeedbackVolume = 0.9,
  _delayMasterVolume = 0.15
) {
  delayTime = _delayTime;
  delayFeedbackVolume = _delayFeedbackVolume;
  delayMasterVolume = _delayMasterVolume;
}
...
export function playLater(
  ...
  soundEffect.buffers.forEach((b) => {
    ...
    if (delayTime != null) {
      ...
      bufferSourceNode.connect(delay);
      delay.connect(soundEffect.gainNode);
      const feedback = audioContext.createGain();
      feedback.gain.value = delayFeedbackVolume;
